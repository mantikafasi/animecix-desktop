 ...
 */
            ...
            electron_updater_1.autoUpdater.on('update-available', function () {
                if (_this.win != null && !_this.win.isDestroyed()) {
                    var notification = new electron_1.Notification({ icon: path_1.default.join(_this.dir, "files", "icon.png"), title: "AnimeciX Güncelleniyor...", body: "Güncelleme indiriliyor" });
                    notification.show();
            ...
            electron_updater_1.autoUpdater.on('update-downloaded', function () {
                var notification = new electron_1.Notification({ icon: path_1.default.join(_this.dir, "files", "icon.png"), title: "AnimeciX Güncelleniyor...", body: "Güncelleme Kuruluyor" });
                notification.show();
    ...
    Main.prototype.sendNotificationIfAllDownloaded = function () {
        ...
        if (this.downloads.filter(function (item) { return (item.status == 'downloading' || item.status == 'queried'); }).length == 0) {
            ...
            var notification = new electron_1.Notification({ icon: path_1.default.join(this.dir, "files", "icon.png"), title: "Tüm indirmeler tamamlandı", body: completed + " dosya indirildi." });
            notification.on('click', function () {
                if (_this.win != null && !_this.win.isDestroyed()) {
                    _this.sendToWindow("showDownloads");
                }
